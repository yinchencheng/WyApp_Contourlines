...
namespace WY_App.Utility
{
    public class IKapLineCam
    {
        ...
        public bool ConfigureFrameGrabber()
        {
            try
            {
                ...
                m_strCamName = Parameters.commministion.productName + "/CamParams.vlcf";
                ...
                ret = IKapBoard.IKapSetInfo(m_hBoard, (uint)INFO_ID.IKP_IMAGE_HEIGHT, Halcon.hv_Height);
                CheckIKapBoard(ret);
                //设置图像缓冲区帧数。
                //
        ...
        public static bool ColseDevice(IKapLineCam grab)
        {
            try
            {
                // 清除回调函数。
        ...
        public static bool OpenDevice(IKapLineCam grab)
        {
            try
            {
                // 初始化 IKapC 运行环境。
                ...
                res = grab.ConfigureFrameGrabber();
                if (!res)
                {
                    return false;
                }
